    ...
    <div className="mt-2 flex flex-wrap items-center gap-2">
      {editing ? (
        ...
        <button
          ...
          className="h-11 rounded-sm px-1 text-sm font-medium tabular-nums text-ink-soft transition-colors hover:bg-surface"
        >
          {minutes}/{target} min
        </button>
      )}
      {QUICK_ADDS.map((quantity) => (
        <button
          ...
          className="h-11 min-w-13 rounded-sm border border-line px-2 text-sm font-medium text-ink transition-colors hover:bg-surface active:bg-surface"
        >
          +{quantity}
        </button>
      ))}
