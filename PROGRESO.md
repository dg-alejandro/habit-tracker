...
**Fase en curso:** Fase 4 — Planificador semanal. El modelo se ha rehecho **cuatro veces** el 2026-07-25 hasta dar con lo que el propietario quería; el de ahora es el bueno y está descrito entero en `CLAUDE.md` §4. **No inventes variantes: lee §4 y respétala.** 302 tests en verde, build limpio, verificado en navegador. Pendiente de la **prueba de aceptación del propietario**, que empieza por lo único que no ha verificado nadie: la sincronización real PC ↔ iPhone con sesión.
...
- **Quedan 23 hallazgos confirmados sin aplicar.** Los tres graves: las columnas de la cuadrícula se aplastan sin tope (falta `minmax`) y el scroll horizontal nunca entra; al solaparse, el chip se parte en 1/N sin suelo; y `weekId` se congela al montar, así que el planificador no se re-basa al cruzar las 4:00. Ninguno pierde datos. El medio que sí puede encadenar en pérdida: **el read-back del push no se trocea**, y con una restauración grande la URL revienta y la cola deja de drenar.
